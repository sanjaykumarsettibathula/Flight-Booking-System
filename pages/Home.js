...
export default function Home() {
  ...
  const { data: flights = [], isLoading: flightsLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return allFlights.slice(0, 10); // Return max 10 flights
    },
  });
  ...
  const { data: wallet, isLoading: walletLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!user) return null;
      const wallets = await base44.entities.UserWallet.filter({
  ...
  const handleBookFlight = async (flight) => {
    ...
    await base44.entities.BookingAttempt.create({
      flight_id: flight.flight_id,
  ...
  const handleConfirmBooking = async (passengerDetails) => {
    ...
    try {
      ...
      const booking = await base44.entities.Booking.create({
        ...
        passenger_name: passengerDetails.passengerName,
        passenger_email: passengerDetails.passengerEmail,
      ...
      const newTransactions = [
        ...(wallet.transactions || []),
        {
          type: "debit",
          amount: currentPrice,
          description: `Flight booking: ${selectedFlight.flight_id}`,
      ...
      await base44.entities.Flight.update(selectedFlight.id, {
        available_seats: Math.max(0, selectedFlight.available_seats - 1),
      });
